...
import json
import sqlite3
from configs.paths import SCHEMAS_PATH, SPIDER_DATABASE_PATH, BIRD_DATABASE_PATH, KAGGLEDBQA_DATABASE_PATH
...
class SchemaBuilder:
    ...
    def __init__(self, dataset:str="spider", db_id:str=None, level:str=None):
        ...
        if level and level != "L0":
            self.db_path = f"data/datasets/{dataset}_{level}/database/{db_id}/{db_id}.sqlite"
        elif dataset == "spider":
        ...
        if level and level != "L0":
            self.out_path = f"{SCHEMAS_PATH}{dataset}_{level}/{db_id}.json"
        else:
            self.out_path = f"{SCHEMAS_PATH}{dataset}/{db_id}.json"
    ...
    def save_schema_json(self):
        ...
        print(f"✅ Schema saved to {self.out_path}")

    # load schema object from json
    def load_schema_json(self, repopulate_attributes=True):
        
        if not os.path.exists(self.out_path):
            raise FileNotFoundError(f"No schema file found at {self.out_path}")
