...
## Ressources
To set up the environment, start by downloading the development sets of [Spider](https://yale-lily.github.io/spider), [BIRD-SQL](https://bird-bench.github.io/) and [KaggleDBQA](https://github.com/Chia-Hsuan-Lee/KaggleDBQA) to the folders `./data/datasets/spider/`, `./data/datasets/bird/dev/` and `./data/datasets/kaggledbqa/`, respectively.  
We are using [Test-Suite-Evaluation](https://github.com/taoyds/test-suite-sql-eval) for Spider and KaggleDBQA evaluation and the official evaluation module for [BIRD-SQL](https://github.com/AlibabaResearch/DAMO-ConvAI/blob/main/bird/llm/src/evaluation.py). Since we modified the original code slightly, the required scripts are already available in `./external/` and you do not have to copy them manually. Make sure to define the OpenAI and TogetherAI API keys in your environment variables as `OPENAI_API_KEY`, `OPENAI_API_ORGANIZATION`, `OPENAI_API_PROJECT` and `TOGETHERAI_API_KEY`. We also recommend using the `dotenv`-package.
