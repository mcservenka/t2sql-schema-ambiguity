...
def execute_sql(predicted_sql,ground_truth, db_path):
    ...
    try:
        ...
        if "no such table" in error_msg:
            return 0, "SCHEMA_TABLE_ERROR"
        elif "no such column" in error_msg:
            return 0, "SCHEMA_COLUMN_ERROR"
        elif "ambiguous column" in error_msg:
            return 0, "SCHEMA_AMBIGUITY_ERROR"
        elif "syntax error" in error_msg:
            return 0, "SYNTAX_ERROR"
        elif "no such function" in error_msg:
            return 0, "DIALECT_ERROR"
        else:
            return 0, "OTHER_RUNTIME_ERROR"
    except:
        return 0, "UNKNOWN_EXECUTION_ERROR"        
    
    if set(predicted_res) == set(ground_truth_res):
        return 1, None
    else:
        return 0, "RESULT_MATCH_ERROR"

from itertools import combinations
from collections import Counter

def soft_execution_acc(predicted_sql, gold_sql, db_path):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    ...
    num_gold_cols = len(gold_rows[0])
    num_pred_cols = len(pred_rows[0])

    # Predicted must have at least as many columns
    if num_pred_cols < num_gold_cols:
        return 0

    gold_counter = Counter(gold_rows)

    # Try all column subsets of predicted result
    for cols in combinations(range(num_pred_cols), num_gold_cols):
        projected = [
            tuple(row[i] for i in cols)
            for row in pred_rows
        ]

        if Counter(projected) == gold_counter:
            return 1
